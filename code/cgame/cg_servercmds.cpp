...
//FIXME: a better way to handle sound-matched strings from other levels (currently uses levelname+sound as key)
	...
	//
	// JKA...
	if (!_stricmp(cgs.stripLevelName[0],"YAVIN1B"))
	{
		strcpy( cgs.stripLevelName[1], "YAVIN1");
	}

/*	// JK2...
	if (!_stricmp(cgs.stripLevelName[0],"KEJIM_BASE") ||
		!_stricmp(cgs.stripLevelName[0],"KEJIM_POST")
		)
	{
		strcpy( cgs.stripLevelName[1], "ARTUS_MINE" );
	}
	if (!_stricmp(cgs.stripLevelName[0],"DOOM_DETENTION") ||
		!_stricmp(cgs.stripLevelName[0],"DOOM_SHIELDS")
		)
	{
		strcpy( cgs.stripLevelName[1], "DOOM_COMM" );
	}
	if (!_stricmp(cgs.stripLevelName[0],"DOOM_COMM"))
	{
		strcpy( cgs.stripLevelName[1], "CAIRN_BAY" );
	}
	if (!_stricmp(cgs.stripLevelName[0],"NS_STARPAD"))
	{
	...
	if (!_stricmp(cgs.stripLevelName[0],"BESPIN_PLATFORM"))
	{
