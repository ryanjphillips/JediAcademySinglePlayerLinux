...
void CStringEdPackage::SetupNewFileParse( LPCSTR psFileName, SE_BOOL bLoadDebug )
{
	...
	m_bLoadingEnglish_ParseOnly = (!_stricmp( m_strLoadingLanguage_ParseOnly.c_str(), "english" )) ? SE_TRUE : SE_FALSE;
...
static char *CopeWithDumbStringData( LPCSTR psSentence, LPCSTR psThisLanguage )
{
	...
	//
	if (!_stricmp(psThisLanguage,"ENGLISH") ||
		!_stricmp(psThisLanguage,"FRENCH") ||
		!_stricmp(psThisLanguage,"GERMAN") ||
		!_stricmp(psThisLanguage,"ITALIAN") ||
		!_stricmp(psThisLanguage,"SPANISH") ||
		!_stricmp(psThisLanguage,"POLISH") ||
		!_stricmp(psThisLanguage,"RUSSIAN")
		)
	{
		char *p;
		...
		if (!Q_stricmpn(sSE_KEYWORD_LANG, psLine, strlen(sSE_KEYWORD_LANG)))
		{
			...
			if ( psReference[0] )
			{
				...
				else
				{
					...
					//
					SE_BOOL bSentenceIsEnglish = (!_stricmp(sThisLanguage,"english")) ? SE_TRUE: SE_FALSE;	// see whether this is the english master or not
					...
					if ( !bSentenceIsEnglish )
					{
						...
						//
						if (_stricmp( m_strLoadingLanguage_ParseOnly.c_str(), sThisLanguage ))
						{
...
void CStringEdPackage::SetString( LPCSTR psLocalReference, LPCSTR psNewString, SE_BOOL bEnglishDebug )
{
	...
	if (itEntry != m_StringEntries.end())
	{
		...
		{				
			...
			//
			if (!_stricmp(psNewString, sSE_EXPORT_SAME))
			{
...
//	_strlwr(sTemp);	// just for consistancy and set<> -> set<> erasure checking etc
...
int SE_GetNumLanguages(void)
{
	...
	//		__DEBUGOUT( "\n" );
			...
			if (!strUniqueStrings.count( psLanguage ))
			{
				...
				//
				if (!_stricmp(psLanguage,"english"))
				{
...
LPCSTR SE_LoadLanguage( LPCSTR psLanguage, SE_BOOL bLoadDebug /* = SE_TRUE */ )
{
	...
	if (psLanguage && psLanguage[0])
	{
		...
		while ( (p=SE_GetFoundFile (strResults)) != NULL && !psErrorMessage )
		{
			...
			if ( !_stricmp( psLanguage, psThisLang ) )
			{
